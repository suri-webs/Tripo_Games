...
export default function Navbar() {
    ...
    const NavLink = ({ href, title, className = "" }: { href: string; title: string; className?: string }) => (
        <Link href={href} className={`text-2xl font-serif h-full flex justify-center items-center w-full transition-all duration-300 hover:bg-[#DA392B] hover:text-white ${className}`}>
            {title}
        </Link>
    );
    ...
    return (
        <nav className="fixed top-0 w-full h-[90px] max-lg:h-[70px] flex justify-between items-start z-50 px-[80px] max-md:px-[10px]">
            ...
            <div className="relative w-[20%] z-99 h-full hidden max-lg:flex justify-center">
                <button onClick={() => setOpen(!open)} className="bg-white flex justify-center items-center h-[80%] w-full rounded-b-2xl z-50">
                ...
                {open && (
                    <ul className="absolute top-full right-0 mt-2 flex flex-col w-[150px] bg-white rounded-lg shadow-lg z-40">
                        {Navbardata.map((item, i) => (
                            <li key={i}>
                                <Link href={item.href} className="block px-7 py-2 text-end hover:bg-[#DA392B] hover:text-white transition duration-300">
                                    {item.NavTittle}
                                </Link>
                            </li>
                        ))}
                        <li className="relative w-[100%] h-[40px] rounded-b-[10px]  group flex justify-end pr-[30px] items-center cursor-pointer hover:bg-[#DA392B] hover:text-white transition-all" onClick={() => {
                            setOpen1(!open1)
                        }}>
                            Games
                            {
                                open1 && <div className="absolute flex flex-col overflow-hidden gap-2 text-black text-[16px] bg-white rounded-b-2xl -bottom-33.5 left-0 w-full z-10 shadow-md">
                                    {NavbarGameData.map((item, i) => (
                                        <Link key={i} href={item.href} className="py-2 text-end pr-[27px] hover:bg-[#DA392B] hover:text-white transition-all">
                                            {item.NavTittle}
                                        </Link>
                                    ))}
                                </div>
